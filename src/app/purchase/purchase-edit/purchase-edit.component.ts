...
export class PurchaseEditComponent implements OnInit {
  ...
  constructor(public db: DatabaseService,public dialog: DialogComponent,private route: ActivatedRoute,private router: Router, public ses: SessionStorage ) 
  { this.date1 = new Date();  }
  ...
  RemoveItem(i, id:any = 0, purchase_id :any = 0)
  {
    ...
    submit_po()
    {
      ...
      this.db.post_rqst( {'po_id':this.purchase_id , 'data':this.purchase_form, 'item':this.purchase_list,'created_by': this.ses.users.id,'created_by_type': this.ses.users.username} , 'purchase/purchaseUpdate')
      .subscribe((result: any) => {
    ...
    delete(i, id){
          this.db.post_rqst( {'id': id, 'po_id':this.purchase_id , 'data':this.purchase_form, 'item':this.purchase_list,'created_by': this.ses.users.id } , 'purchase/deletePurchaseItem')
          .subscribe( r => {
            this.itemdetail.splice(i,1);
            this.dialog.success( 'Purchase Order Item Deleted Successfully!');
            this.getPurchaseDetails(this.purchase_id);
