...
sub_child:any = [];
  ...
  challan_list()
  {
    ...
    this.db.post_rqst(  {'id':this.order_id}, 'sales/sale_invoice_detail')
      .subscribe( d  => {
        console.log( d );
    ...
    invoiceReceive()
    {
     ...
     this.db.insert_rqst( {'items': this.sub_child , 'created_by' : this.db.datauser.id , 'remark' : this.parent.remark},'sales/invoiceReceive')
      .subscribe( r => {
        this.loading_list = true;
        this.dialog.success('Invoice Stock Received Successfully!');
        this.challan_list();
