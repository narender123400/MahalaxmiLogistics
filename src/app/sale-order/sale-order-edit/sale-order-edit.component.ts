...
export class SaleOrderEditComponent implements OnInit {
  ...
  constructor(public db: DatabaseService,public dialog: DialogComponent,private route: ActivatedRoute,private router: Router) 
  { this.date1 = new Date();  }
  
  ngOnInit() {
    this.route.params.subscribe(params => {
      
      this.sale_order_id = params['id'];
  ...
  getagreement() {
    ...
    this.db.post_rqst( {'id' :this.sale_order_id} , 'sales/getAgreementDetail')
    .subscribe((d: any) => {
      this.loading_list = false;
      
      this.aggreement  = d.argeement; 
      this.argeement_item  = d.argeement_item; 
      this.sales_order_consignee  = d.sales_order_consignee; 

      this.orderData = this.aggreement;

      this.orderItemList = this.argeement_item;

      for (let i = 0; i < this.argeement_item.length; i++) {
        this.orderItemList[i].qty = this.argeement_item[i].item_qty;
      }

      this.orderData.consigner_id =   this.aggreement.consigner_id;

  for (let i = 0; i < this.sales_order_consignee.length; i++) {
    this.agreement_consignee.push( this.sales_order_consignee[i].consignee_id );
    ...
    //                                     
      ...
      // this.agreement_consignee  =  this.sales_order_consignee.consignee_id;
    //   console.log(this.consing                                           erList2);
    //   for (let i = 0; i < this.consingerList2.length; i++) {
    //     for (let j = 0; j < this.sales_order_consignee.length; j++) {
    //      if(this.consingerList2[i].id ==this.sales_order_consignee.consignee_id){
    //       this.consingerList2[i].check=true;
    //      }else{
    //        ''
    //      }
    //   }
    ...
    },err => {   this.loading_list = false; this.dialog.retry().then((result) => {  this.getagreement();  }); });
  }
  ...
  getConsingersList() {
    ...
    this.db.post_rqst({'branch_id': this.orderData.branch_id  , 'sales_id':this.sale_order_id} , 'sales/getConsinger?&s='+ this.search)
    .subscribe( r => {
      console.log(r);
      this.loading_list = false;
  ...
  RemoveItem(i, id:any = 0, sales_id :any = 0)
  {
    console.log(i);
    ...
    this.dialog.delete('Item Data !').then((result) => {
        ...
        this.db.post_rqst( {'id': id, 'sales_id' : this.sale_order_id ,'data':this.orderData ,'totitem':this.orderItemList[i].qty ,'amount':this.orderItemList[i].amount } , 'sales/deleteAgreementItem')
        .subscribe( r => {
          this.orderItemList.splice(i,1);
        });
...
//     }
  ...
  submit_sales_order()
  {
    ...
    this.db.insert_rqst( {'data':this.orderData, 'id':this.sale_order_id} , 'sales/updateAgreement')
    .subscribe((result: any) => {
      this.savingData = false;
      this.loading_list = false;
      console.log(result);
      this.router.navigate(['/sale-order-list']);
      this.dialog.success( 'Aggrement Update Successfully!');
