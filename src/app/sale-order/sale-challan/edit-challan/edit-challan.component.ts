...
export class EditChallanComponent implements OnInit {
  ...
  ngOnInit() {   
    ...
    console.log( this.itemdetail );
      ...
      this.challan_id = params['id'] || 0;
      this.branch_id = params['branch_id'] || 0;
      console.log(this.challan_id);        
    ...
    if (this.challan_id) 
      this.getPurchaseDetails(this.challan_id); 
...
this.data.accept_qty = parseInt(  this.data.qty );
  ...
  orderdetail:any = {};  
  tmp:any = {};  
  getPurchaseDetails(challan_id) {
    console.log( 'getPurchaseDetails');
    if(this.challan_id == 0)return;
    this.loading_list = false;
    this.db.get_rqst(  '', 'sales/getChallanDetail/' + challan_id )
    .subscribe(d => {
      this.loading_list = true;
console.log(d);
...
console.log( d.orderdetail );
      ...
      this.purchase_form = d.argeement;
      if(d.argeement.status == 'Received' ){
      this.dialog.warning('Received Challan Not Editable!');
      this.db.goBack();
      return;
      }
      ...
      this.loading_list = true;
              ...
              this.dialog.retry().then((result) => {  this.getPurchaseDetails(challan_id); }); });
  }
  ...
  add_receive_order_detail()
  {
   ...
   this.formData.challan_id = this.challan_id;
   this.formData.branch_id = this.branch_id;
   
   this.db.insert_rqst( {'challan' : this.formData },'sales/update_sale_challan')
    .subscribe( r => {
      ...
      this.router.navigate(['/sale-order-detail/'+this.challan_id]); 
    }
