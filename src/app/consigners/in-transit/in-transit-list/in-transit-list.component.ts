...
@Component({
  selector: 'app-in-transit-list',
  templateUrl: './in-transit-list.component.html'
})
export class InTransitListComponent implements OnInit {
  ...
  getProductList() {
    ...
    this.db.post_rqst(  {'consigner_id':this.franchise_id , 'filter' : this.filter }, 'sales/consigner_intransit?page=' + this.current_page + '&s=' + this.search + '&t=' + typ)
      .subscribe(data => {
        console.log(data);
